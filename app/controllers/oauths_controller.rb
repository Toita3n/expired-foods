class OauthsController < ApplicationController
  ...
  def update_from(provider_name)
    ...
    current_user.update(attrs.except(:name)) # current_userのオブジェクト情報の更新
    current_user.add_provider_to_user(provider_name.to_s, @user_hash[:uid].to_s) #　プロバイダーの情報をユーザーに追加
